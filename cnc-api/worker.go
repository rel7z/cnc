...
// executeShellTask runs the command from the task payload.
// The server already substituted {input} with the chunk path before dispatch.
func (w *WorkerAgent) executeShellTask(task *Task) (*TaskResult, error) {
	...
	// NoTimeout sentinel (-1): run without a deadline.
	// Zero or any other non-positive value falls back to DefaultTimeout.
