...
func main() {
	...
	flag.StringVar(&ipInput, "ip", "", "Single IP or comma-separated IPs")
	flag.StringVar(&fileInput, "file", "", "File containing IPs (one per line)")
	flag.StringVar(&fileInput, "f", "", "File containing IPs (one per line)")
	flag.StringVar(&fileInput, "l", "", "File containing IPs (one per line)")
	flag.IntVar(&concurrency, "c", runtime.NumCPU()*4, "Number of concurrent workers")
	flag.DurationVar(&timeout, "timeout", 30*time.Second, "HTTP timeout per request")
	flag.StringVar(&apiKey, "key", "", "API key (optional)")
	flag.StringVar(&apiKey, "api-key", "", "API key (optional)")
	flag.IntVar(&rateLimit, "rate", 0, "Rate limit (requests/sec, 0 = unlimited)")
	flag.StringVar(&outputFile, "out", "result.txt", "Output file for domains")
	flag.StringVar(&outputFile, "o", "result.txt", "Output file for domains")
	flag.Parse()
	...
	if len(ips) == 0 {
		fmt.Fprintln(os.Stderr, "Usage: reverseip -ip 1.1.1.1 [-file ips.txt | -l ips.txt] [-c 100] [-out result.txt]")
		os.Exit(1)
	}
