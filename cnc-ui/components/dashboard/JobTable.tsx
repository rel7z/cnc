...
function ModeBadge({ mode }: { mode: string }) {
  let colorClass = "bg-purple-500/15 text-purple-400 ring-purple-500/30";
  if (mode === "spread") {
    colorClass = "bg-blue-500/15 text-blue-400 ring-blue-500/30";
  } else if (mode === "server") {
    colorClass = "bg-amber-500/15 text-amber-400 ring-amber-500/30";
  }
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ring-1 ${colorClass}`}
    >
      {mode}
    </span>
  );
  ...
  if (jobList.length === 0) {
    return (
      <EmptyState
        ...
        description="Submit a job with: cnc spread <file> <command>"
      />
    );
  }
