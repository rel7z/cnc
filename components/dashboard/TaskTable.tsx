...
export function TaskTable({ jobId }: TaskTableProps) {
  ...
  if (taskList.length === 0) {
    return (
      <EmptyState
        title="No tasks yet"
        description="Tasks appear here once the server splits the input file"
      />
    );
  }
      ...
      <table className="w-full text-sm text-left">
        ...
        <tbody className="divide-y divide-gray-800">
          {taskList.map((task) => (
            ...
            >
              <td className="py-3 px-4 font-mono text-xs text-gray-400 max-w-[180px]">
                <span className="truncate block" title={task.id}>
                  {task.id.split("_").slice(-1)[0]
                    ? `…_${task.id.split("_").slice(-1)[0]}`
                    : task.id}
                </span>
              </td>
